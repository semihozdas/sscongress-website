...
const FEATURE_ICONS = [Award, Shield, Zap, Target];
  ...
  return (
    ...
    >
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', position: 'relative', zIndex: 10 }}>
        ...
        >
          <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.25em', textTransform: 'uppercase', color: EM, marginBottom: 14 }}>{t('whyus.badge')}</p>
          <h2 style={{ fontFamily: 'Outfit', fontWeight: 900, fontSize: 'clamp(2rem,4.5vw,3.2rem)', letterSpacing: '-0.04em', lineHeight: 1.05, color: 'var(--c-heading)' }}>
            {t('whyus.h2_pre')}{' '}
            <span style={{ background: 'linear-gradient(135deg,#6ee7b7,#10b981)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>{t('whyus.h2_grad')}</span>{' '}
            {t('whyus.h2_post')}
          </h2>
        </motion.div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 20, marginBottom: 80 }} className="whyus-grid">
          {FEATURE_ICONS.map((Icon, i) => (
            ...
            >
              ...
              <h3 style={{ fontFamily: 'Outfit', fontWeight: 700, fontSize: 15, color: 'var(--c-heading)', marginBottom: 10, lineHeight: 1.35 }}>{t(`whyus.feat${i+1}.title`)}</h3>
              <p style={{ fontSize: 13, color: 'var(--c-muted)', lineHeight: 1.7 }}>{t(`whyus.feat${i+1}.desc`)}</p>
            </motion.div>
          ))}
        </div>
        ...
        <div style={{ borderTop: BORDER, paddingTop: 64, display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 32 }} className="whyus-stats">
          {[{ target: 200, k: 'whyus.stat_client' }, { target: 25, k: 'whyus.stat_country' }, { target: 50, k: 'whyus.stat_forum' }, { target: 10, k: 'whyus.stat_year' }].map(({ target, k }, i) => (
            <motion.div key={k} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 11, color: 'var(--c-subtle)', marginTop: 8, textTransform: 'uppercase', letterSpacing: '0.15em', fontWeight: 500 }}>{t(k)}</div>
            </motion.div>
          ))}
        </div>
      </div>
