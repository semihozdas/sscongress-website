...
const completed = [
  { title: 'TURKISH-AFRICAN INVESTMENT SUMMIT', location: 'Istanbul, Turkiye' },
  { title: 'TURK & QATAR BUSINESS FORUM', location: 'Doha, Katar' },
  { title: 'Suudi Arabistan Is Forumu', location: 'Riyad, Suudi Arabistan' },
  { title: 'London Halal Forum', date: '20-21 Kasim 2025', location: 'Excel London, UK' },
  { title: 'Makkah Halal Forum', date: '14-16 Subat 2026', location: 'Mekke, Suudi Arabistan' },
  { title: 'Driving Urban Transitions (DUT)', date: '9 Eylul 2025' },
  { title: 'Antalya Ticaret Borsasi Arnavutluk Programi' },
];

const upcoming = [
  { title: '17. Uluslararasi Ekonomi Forumu — Kazan Forum 2026', date: '12-17 Mayis 2026', link: '/proje/kazan-forum-2026', highlight: true },
];
    ...
    <div style={{ backgroundColor: 'var(--c-bg)', minHeight: '100vh', paddingTop: '8rem', paddingBottom: '6rem' }}>
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 1.5rem' }}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: '4rem' }}>
          <p style={{ color: '#10b981', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.3em', marginBottom: '1rem' }}>Portfoyu</p>
          <h1 style={{ fontFamily: 'Outfit', fontSize: 'clamp(2.5rem, 6vw, 4rem)', fontWeight: 900, color: 'var(--c-heading)', letterSpacing: '-0.03em', lineHeight: 1.05 }}>Projelerimiz</h1>
        </motion.div>
        ...
        <div style={{ marginBottom: '4rem' }}>
          ...
          {upcoming.map((p, i) => (
            <motion.div key={i} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <Link to={p.link} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1.5rem', padding: '2rem', borderRadius: '1rem', border: '1px solid rgba(16,185,129,0.25)', backgroundColor: 'rgba(16,185,129,0.05)', textDecoration: 'none', flexWrap: 'wrap' }}>
                <div>
                  <Tag label="Aktif" color="#10b981" bg="rgba(16,185,129,0.25)" />
                  <h3 style={{ fontFamily: 'Outfit', fontSize: '1.25rem', fontWeight: 800, color: 'var(--c-heading)', marginTop: '0.75rem', marginBottom: '0.5rem' }}>{p.title}</h3>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#10b981', fontSize: '0.875rem', fontWeight: 600 }}>
                    <Calendar size={15} /> {p.date}
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#10b981', fontWeight: 700, fontSize: '0.9rem', whiteSpace: 'nowrap' }}>
                  Detaylari Incele <ArrowRight size={16} />
                </div>
              </Link>
            </motion.div>
          ))}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1rem' }}>
            {completed.map((p, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.06 }} style={cardBase}>
                <Tag label="Tamamlandi" color="#22c55e" bg="rgba(34,197,94,0.1)" />
