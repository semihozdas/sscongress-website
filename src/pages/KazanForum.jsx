...
const reasons = [
  'Stratejik is ortakliklari kurma firsati',
  'Kuresel pazarda gorunurluk artisi',
  'Ust duzey diplomatik ve ticari aglara erisim',
  'Yeni pazarlara giris danismanligi destegi',
];
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div style={{ minHeight: '280px', background: 'linear-gradient(135deg, #0a1410, #1e1006)', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
            ...
            <div style={{ position: 'relative', zIndex: 10, textAlign: 'center', padding: '3rem 2rem' }}>
              <p style={{ color: '#10b981', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.3em', marginBottom: '1rem' }}>17. Uluslararasi Ekonomi Forumu</p>
          ...
          <div style={{ padding: '2.5rem', display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '3rem', alignItems: 'start' }}>
            <div>
              <h2 style={{ fontFamily: 'Outfit', fontSize: '1.75rem', fontWeight: 800, color: 'var(--c-heading)', marginBottom: '1.25rem' }}>Etkinlik Detayi</h2>
              <p style={{ color: '#94a3b8', lineHeight: 1.8, fontSize: '1rem', marginBottom: '1.75rem' }}>
                17. Uluslararasi Ekonomi Forumu Rusya — Islam Dunyasi: Kazan Forum 2026, kuresel ekonominin en onemli bulusma noktalarindan biri olarak Islam dunyasi ile Rusya arasindaki stratejik is birliklerini guclendir meyi hedeflemektedir.
              </p>
              <div style={{ padding: '1.5rem', borderRadius: '0.875rem', backgroundColor: 'var(--c-bg-alt)', border: '1px solid var(--c-border)', marginBottom: '1.75rem' }}>
                <h3 style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontFamily: 'Outfit', fontSize: '1rem', fontWeight: 700, color: 'var(--c-heading)', marginBottom: '0.75rem' }}>
                  <Info size={18} style={{ color: '#10b981' }} /> Mevcut Durum
                </h3>
                <p style={{ color: '#94a3b8', lineHeight: 1.7, fontSize: '0.9rem' }}>
                  Organizasyon ve katilim hazirliklari SS Congress ekibi tarafindan titizlikle surdurul mektedir. Sektorel delegasyon katilim surecleri ve ikili is gorusmeleri (B2B) ajandasi planlanmaktadir.
                </p>
              </div>
              <h3 style={{ fontFamily: 'Outfit', fontSize: '1.1rem', fontWeight: 700, color: 'var(--c-heading)', marginBottom: '1rem' }}>Neden Katilmalisiniz?</h3>
              <ul style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {reasons.map((r, i) => (
                  <li key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', color: '#94a3b8', fontSize: '0.95rem' }}>
            ...
            <div style={{ position: 'sticky', top: '7rem', padding: '2rem', borderRadius: '1rem', border: '1px solid rgba(255,255,255,0.07)', backgroundColor: 'rgba(16,185,129,0.04)' }}>
              <h4 style={{ fontFamily: 'Outfit', fontWeight: 700, color: 'var(--c-heading)', marginBottom: '1.5rem', paddingBottom: '1rem', borderBottom: '1px solid rgba(0,0,0,0.05)', fontSize: '1rem' }}>
                Etkinlik Bilgileri
              </h4>
              {[
                { Icon: Calendar, label: 'Tarih', value: '12 - 17 Mayis 2026' },
                { Icon: MapPin, label: 'Konum', value: 'Kazan, Tataristan / Rusya' },
                ...
                <div key={label} style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
                  ...
                  <div>
                    <p style={{ fontSize: '0.7rem', fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.2rem' }}>{label}</p>
                    <p style={{ fontWeight: 700, color: 'var(--c-heading)', fontSize: '0.9rem' }}>{value}</p>
                  </div>
                </div>
              ))}
