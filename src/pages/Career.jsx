...
export default function CareerPage() {
  const { t } = useLanguage();
  const reqs = Array.from({ length: REQ_COUNT }, (_, i) => t(`career.req${i+1}`));
  return (
    <div style={{ backgroundColor: 'var(--c-bg)', minHeight: '100vh', paddingTop: 100, paddingBottom: 96 }}>
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '0 32px' }}>
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} style={{ marginBottom: 56 }}>
          <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.25em', textTransform: 'uppercase', color: EM, marginBottom: 14 }}>{t('career.badge')}</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr' }} className="career-card">
            <div style={{ padding: '40px 32px', background: 'var(--c-bg-alt)', borderRight: BORDER }}>
              ...
              <div style={{ marginTop: 24, padding: '10px 16px', borderRadius: 8, background: 'var(--c-border-md)', border: BORDER, display: 'inline-block' }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: EM, textTransform: 'uppercase', letterSpacing: '0.1em' }}>{t('career.status')}</span>
              </div>
            </div>

            <div style={{ padding: '40px 36px' }}>
              <h3 style={{ fontFamily: 'Outfit', fontWeight: 700, fontSize: 16, color: 'var(--c-heading)', marginBottom: 12 }}>{t('career.desc_title')}</h3>
              <p style={{ fontSize: 14, color: MUTED, lineHeight: 1.7, marginBottom: 28 }}>
                {t('career.desc')}
              </p>
              <h3 style={{ fontFamily: 'Outfit', fontWeight: 700, fontSize: 16, color: 'var(--c-heading)', marginBottom: 16 }}>{t('career.reqs_title')}</h3>
        ...
        >
          <h3 style={{ fontFamily: 'Outfit', fontWeight: 700, fontSize: 18, color: 'var(--c-heading)', marginBottom: 10 }}>{t('career.open_title')}</h3>
          <p style={{ color: MUTED, fontSize: 14, marginBottom: 16 }}>{t('career.open_desc')}</p>
